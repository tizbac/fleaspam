...
do -- load config file
local CONFIG_FILE
  CONFIG_FILE  = "LuaRules/Configs/spawn_defs_fleas.lua"
...
SetGameRulesParam("queens",      queens)
for unitName in pairs(chickenTypes) do
   if (cenabled == 1) then 
...
local JUNO = {}
local KROW_ID = UnitDefNames["armfleah"].id
local OVERSEER_ID = UnitDefNames["armfleaf"].id --chickenh1h
local KROW_LASER = "krow_laser_index"
local SMALL_UNIT = UnitDefNames["armfleab"].id
local MEDIUM_UNIT = UnitDefNames["armfleac"].id
local LARGE_UNIT = UnitDefNames["armfleah"].id
